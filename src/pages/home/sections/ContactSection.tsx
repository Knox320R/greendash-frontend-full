  ...
  <section className="py-20 bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-green-50 via-white to-green-50">
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div className="text-center mb-10">
        <h2 className="text-2xl font-bold text-gray-700 mb-8 bg-gradient-to-b from-gray-50 to-white">{t('home:contact.title')}</h2>
        <p className="text-lg text-gray-500">{t('home:contact.subtitle')}</p>
      </div>
      <form className="bg-white rounded-md shadow-lg p-8 space-y-6">
        <div>
          <label htmlFor="contact-title" className="block text-sm font-medium text-gray-700 mb-1">{t('home:contact.form.name')}</label>
          <motion.input
            id="contact-title"
            name="title"
            type="text"
        ...
        <div>
          <label htmlFor="contact-message" className="block text-sm font-medium text-gray-700 mb-1">{t('home:contact.form.message')}</label>
          <motion.textarea
            id="contact-message"
            name="message"
            rows={5}
        ...
        <div className="text-center">
          ...
          {!isAuthenticated && (
            <div className="mt-2 text-sm text-red-600">{t('common:login')} to send a message.</div>
          )}
        </div>
      </form>
      {supportEmail && (
        <div className="mt-6 text-center text-sm text-gray-500">
          {t('home:contact.email')} <a href={`mailto:${supportEmail}`} className="text-green-600 underline">{supportEmail}</a>
        </div>
      )}
    </div>
  </section>
